...
public interface PagamentoRepository extends JpaRepository<Pagamento, Long> {
    ...
    @Query(value = """
            ...
            INNER JOIN enrolment e
                ON e.ph_enrolment = p.enrolment_pk

            INNER JOIN student s
                ON s.pk_student = e.student_pk

            INNER JOIN fee f
                ON f.ph_fee = p.fee_pk

            INNER JOIN cash_box cb
                ON cb.ph_cash_box = p.cash_box_pk
    ...
    @Query("""
            SELECT new com.SistemSchool.modulo_secrtaria.dto.PagamentoDTO(
                p.pkPagamento,
                p.numeroDocumento,
                p.enrolment.phEnrolment,
                p.enrolment.enrolmentNumer,
                p.enrolment.student.fullName,
                p.fee.phFee,
                null,
                p.cashBox.phCashBox,
                p.cashBox.cashBoxNumber,
                p.valor,
                p.desconto,
                p.multa,
                p.total,
    ...
    @Query("""
            SELECT SUM(p.total)
            FROM Pagamento p
            WHERE p.estado = com.SistemSchool.modulo_secrtaria.io.EstadoPagamento.PAGO
            """)
    java.math.BigDecimal getTotalConfirmado();
