 ...
 *   • A4 Portrait   → Relatórios por turma
...
public final class PdfReportService {
    ...
    /* ═══════════════════════════════════════════════════════════════
       A5  →  Ficha Individual de Matrícula (impressão)
       ═══════════════════════════════════════════════════════════════ */
    public static byte[] generateEnrolmentReport(EnrolmentDTO enrolment, StudentDTO student)
            throws DocumentException {
        Document doc = new Document(PageSize.A5, 30, 30, 25, 25);
        ByteArrayOutputStream baos = new ByteArrayOutputStream();
        PdfWriter.getInstance(doc, baos);

        doc.open();
        doc.add(buildFichaHeader());
        doc.add(spacerSmall());
        doc.add(buildTopInfoTable(enrolment));
        doc.add(spacer());

        addSectionTitle(doc, "DADOS PESSOAIS");
        doc.add(buildFichaPersonalDataTable(student));
        doc.add(spacer());

        doc.add(buildFiliacaoTable(student));
        doc.add(spacer());

        addSectionTitle(doc, "OBSERVAÇÃO");
        doc.add(buildObservacaoSection(student));
        doc.add(spacer());
        doc.add(spacer());
        doc.add(buildSignatureTable());
    ...
    public static byte[] generateEnrolmentCardA6(EnrolmentDTO enrolment, StudentDTO student)
            throws DocumentException {
        // A6 = 105 x 148 mm  →  297.64 x 419.53 pts (1 mm = 2.8346 pts)
        ...
        footer.setSpacingBefore(8f);  // ← CORRIGIDO: setSpacingBefore em vez de setSpacingTop
        doc.add(footer);
    ...
    /* ═══════════════════════════════════════════════════════════════
       FICHA DO(A) ALUNO(A)  →  blocos do formulário físico (A5)
       ═══════════════════════════════════════════════════════════════ */

    private static PdfPTable buildFichaHeader() {
        ...
        Paragraph schoolName = new Paragraph("COMPLEXO ESCOLAR PRIVADO EDUCAÇÃO PARA TODOS", F_SCHOOL);
        schoolName.setAlignment(Element.ALIGN_CENTER);
        Paragraph fichaTitle = new Paragraph("FICHA DO(A) ALUNO(A)", F_FICHA);
        ...
        PdfPCell photoCell = new PdfPCell(new Phrase("FOTO", F_PHOTO));
        photoCell.setBorder(Rectangle.BOX);
    ...
    private static PdfPTable buildTopInfoTable(EnrolmentDTO e) {
        ...
        String tipo = e.getEnrolmentType() != null ? e.getEnrolmentType().toString().toUpperCase() : "";
        addCheckboxCell(table, "MATRÍCULA", tipo.contains("MATRIC"));
        addCheckboxCell(table, "CONFIRMAÇÃO", tipo.contains("CONFIRM"));

        addPlainFieldCell(table, "CLASSE", e.getSchoolclassnome());
        addPlainFieldCell(table, "Nº DA FICHA", e.getEnrolmentNumer());
        addPlainFieldCell(table, "PERÍODO", e.getShift());
        addPlainFieldCell(table, "ANO LECTIVO", currentAcademicYear());

        return table;
    }

    private static PdfPTable buildFichaPersonalDataTable(StudentDTO s) {
        PdfPTable table = new PdfPTable(1);
        table.setWidthPercentage(100);
        table.setSpacingAfter(4f);

        addFullLineCell(table, "NOME", s.getFullName());
        addFullLineCell(table, "Nº DE IDENTIFICAÇÃO", s.getBiNumber());
        addFullLineCell(table, "DATA DE NASCIMENTO", s.getNascDate());
        addFullLineCell(table, "NATURAL", s.getAddressProvice());
        addFullLineCell(table, "MORADA", s.getAddressStreet());
        addFullLineCell(table, "GÉNERO", s.getGender());
        addFullLineCell(table, "DATA DE EMISSÃO", LocalDate.now());
        return table;
    }

    private static PdfPTable buildFiliacaoTable(StudentDTO s) {
        PdfPTable table = new PdfPTable(1);
        table.setWidthPercentage(100);
        table.setSpacingAfter(4f);

        addFullLineCell(table, "NOME DO PAI", s.getNameFather());
        addFullLineCell(table, "NOME DA MÃE", s.getNameMather());

        String contacto = (s.getPhone_1() != null && !s.getPhone_1().trim().isEmpty())
                ? s.getPhone_1() : s.getPhone_2();
        addFullLineCell(table, "CONTACTO DO ENCARREGADO", contacto);
        return table;
    }

    private static PdfPTable buildObservacaoSection(StudentDTO s) {
        PdfPTable table = new PdfPTable(1);
        table.setWidthPercentage(100);

        addObservacaoQuestionRow(table, "POSSUI ALGUM PROBLEMA DE SAÚDE?");
        addQualLine(table);
        addObservacaoQuestionRow(table, "TEM ALGUMA DEFICIÊNCIA?");
        addQualLine(table);
        addObservacaoQuestionRow(table, "POSSUI ALGUMA ALERGIA?");
        addQualLine(table);

        if (s.getObs() != null && !s.getObs().trim().isEmpty()) {
            addFullLineCell(table, "OUTRAS OBSERVAÇÕES", s.getObs());
        }
        return table;
    }

    private static PdfPTable buildSignatureTable() {
        PdfPTable table = new PdfPTable(2);
        ...
        table.addCell(emptyCell(28f));
        table.addCell(emptyCell(28f));
        table.addCell(signatureLabelCell("O ENCARREGADO"));
        table.addCell(signatureLabelCell("O FUNCIONÁRIO"));
        return table;
    }
